...
public final class GreatCircle
{
  ...
  private List<GreatCircleWayPoint> route;
...
//    System.out.println("Ari:" + ari);
      ...
      route.add(new GreatCircleWayPoint(smallStart, arrival.equals(smallStart) ? null : new Double(ari)));
  ...
  public List<GreatCircleWayPoint> getRoute()
  {
    return route;
  }
...
//  GeoPoint p = dr(start, 30D, 230D);
    ...
    List<GreatCircleWayPoint> routeGC = test.getRoute();
    routeGC.stream()
        .forEach(gcPt -> {
          System.out.println(String.format("Z: %.01f\272, %s", gcPt.getZ(), gcPt.getPoint().radiansToDegrees().toString()));
        });

    System.out.println("GC Test 2:");
    start = new GeoPoint(Math.toRadians(37.743865), Math.toRadians(-122.5072902));
//  GeoPoint p = dr(start, 30D, 230D);
    p = new GeoPoint(Math.toRadians(19.7373), Math.toRadians(-155.0668648));
    System.out.println("Arriving:" + GeomUtil.decToSex(Math.toDegrees(p.getL()), GeomUtil.SWING, GeomUtil.NS) + ", " + GeomUtil.decToSex(Math.toDegrees(p.getG()), GeomUtil.SWING, GeomUtil.EW));
    ...
    System.out.println("Dist: rhumbline:" + dist + " (GC:" + gcDist + "), route:" + Math.toDegrees(route));
