  ...
  <xsl:template name="table1" match="//table[@id=1]">
    ...
    <table border="1">
      <xsl:for-each select="min">
        <xsl:if test="position() = 1">
          ...
          <tr>
            ...
            <xsl:for-each select="value[@deg &gt;= $inf and @deg &lt;= ($inf + 14)]">                  
              <td align="center"><b><xsl:value-of select="./@deg"/>&#176;</b></td>                    
            </xsl:for-each>
            <td>&nbsp;</td>
          </tr>
        </xsl:if>
        <tr>
          <!-- First column -->
          <td><b><xsl:value-of select="./@val"/>&#39;</b></td>
          <!-- data -->
          <xsl:for-each select="value[@deg &gt;= $inf and @deg &lt;= ($inf + 14)]">                  
            <td><xsl:value-of select="."/></td>                    
          </xsl:for-each>
          <!-- Last column -->
          <td align="right"><b><xsl:value-of select="60 - ./@val"/>&#39;</b></td>
        </tr>
        <xsl:if test="position() = last()">
          ...
          <tr>
            <td>&nbsp;</td>
            <xsl:for-each select="value[@deg &gt;= $inf and @deg &lt;= ($inf + 14)]">                  
              <td align="center"><b><xsl:value-of select="359 - ./@deg"/>&#176;</b></td>                    
            </xsl:for-each>
            <td>&nbsp;</td>
          </tr>
        </xsl:if>
    ...
    <address>page <xsl:value-of select="($inf div 15) -11"/></address>
  </xsl:template>
</xsl:stylesheet>
