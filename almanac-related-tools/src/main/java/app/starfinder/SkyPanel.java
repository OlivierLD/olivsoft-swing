...
public class SkyPanel
	...
	public SkyPanel() {
		try {
			deltaT = Float.parseFloat(System.getProperty("deltaT", "65.5"));
	...
	private void jbInit() throws Exception {
		try {
			...
			try {
				latitude = Double.parseDouble(props.getProperty("last.latitude"));
			} catch (Exception ignore) {
			}
			try {
				longitude = Double.parseDouble(props.getProperty("last.longitude"));
			} catch (Exception ignore) {
			}
