...
public class Utils {
    ...
    public static Calendar decimalHourToDate(double decHour) {
        return decimalHourToDate(decHour, TimeZone.getTimeZone("Etc/UTC"));
    }

    public static Calendar decimalHourToDate(double decHour, TimeZone tz) {
        int h = (int) Math.floor(decHour);
        int m = (int) Math.floor((decHour - h) * 60);
        int s = (int) Math.round(3600d * ((decHour - h) - (m / 60d)));
