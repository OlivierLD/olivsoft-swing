...
import polarmaker.polars.smooth.gui.components.polars.CoeffForSection;
...
//      System.out.println("-------------------");
    ...
    {
      Vector<PolarPoint> v = new Vector<PolarPoint>();
...
//    System.out.println("Smoothing Data");
      ...
      for (int i=0; i<selectedNode.length; i++)
      {
        ...
        else if (selectedNode[i].getType() == PolarTreeNode.SECTION_TYPE)
        {
          ...
          if (coeffMap == null)
            coeffMap = new HashMap<String, CoeffForPolars>();
