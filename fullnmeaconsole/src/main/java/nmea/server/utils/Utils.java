        ...
        } else if ("MWV".equals(sentenceId)) // Apparent Wind Speed and Direction
        {
            ...
            if (false && ndc != null) {
                if (current != null) {
                    ...
                    try {
                        ((Map<Long, NMEADataCache.CurrentDefinition>) ndc.get(NMEADataCache.CALCULATED_CURRENT)).put(0L, new NMEADataCache.CurrentDefinition(0L, new Speed(current.speed), new Angle360(current.angle)));
                    } catch (Exception ex) {
...
//  System.out.println("AWS:" + aws + ", TWS:" + tws + ", AWA:" + awa + ", TWA:" + twa);
        ...
        try {
            ...
            if (speakUp) System.out.println("MaxLeeway not available:" + ex.toString());
        }
        ...
        try {
            ...
            try {
                try {
                    ...
                    if (speakUp) System.err.println("MAX_LEEWAY:" + npe.getLocalizedMessage());
                }
                try {
                    awsFactor = ((Double) NMEAContext.getInstance().getCache().get(NMEADataCache.AWS_FACTOR)).doubleValue();
                } catch (NullPointerException npe) {
                    if (speakUp) System.err.println("AWS_FACTOR:" + npe.getLocalizedMessage());
                }
                try {
                    awaOffset = ((Double) NMEAContext.getInstance().getCache().get(NMEADataCache.AWA_OFFSET)).doubleValue();
                } catch (NullPointerException npe) {
                    if (speakUp) System.err.println("AWA_OFFSET:" + npe.getLocalizedMessage());
                }
                try {
                    bspFactor = ((Double) NMEAContext.getInstance().getCache().get(NMEADataCache.BSP_FACTOR)).doubleValue();
                } catch (NullPointerException npe) {
                    if (speakUp) System.err.println("BSP_FACTOR:" + npe.getLocalizedMessage());
                }
                try {
                    hdgOffset = ((Double) NMEAContext.getInstance().getCache().get(NMEADataCache.HDG_OFFSET)).doubleValue();
                } catch (NullPointerException npe) {
                    if (speakUp) System.err.println("HDG_OFFSET:" + npe.getLocalizedMessage());
                }
                ...
                if (line == null)
                    ...
                    if (line.startsWith("$") && line.length() > 6) {
                        if (StringParsers.validCheckSum(line)) {
                            ...
                            try {
                                Utils.parseAndCalculate(key, line, ndc);
                            } catch (Exception ex) {
                                if (speakUp)
