...
public class TransmissionSelectorFrame
     extends JFrame
  implements DisplayMapRequestor,
             DisplayTxRequestor
{
  private final static String SKEDFILE_NAME = "SkedFile.txt";
  ...
  public TransmissionSelectorFrame()
  {
    this(null);
  }
  public TransmissionSelectorFrame(String dataFile)
  {
    ...
    throws Exception
  {
    ...
    this.setTitle( "Transmission Selector" );
    try { this.setIconImage(new ImageIcon(this.getClass().getResource("script.png")).getImage()); } catch (Exception ignore) {}
    statusBar.setText( "" );
  ...
  private String getTabName(ReadTransmissionData.BulletinData bd)
  {
    ...
    while (si.hasNext())
    {
      ret += ((ret.length()>0?"/":"") + ((StationData)si.next()).callsign);
    }
    return ret;
  }
  ...
  public void displayChart(String cs,
                           ...
                           double right)
  {
    chartCommandPanel.setChartToEnhance(cs + " " + name, top, bottom, left, right);
  }
  ...
  public void switchToTxPanel(String[] txData)
  {
    ...
    while (iterator.hasNext())
    {
      ReadTransmissionData.BulletinData bd = (ReadTransmissionData.BulletinData)iterator.next();
...
//      System.out.println(callsign);
        ...
        while (freqs.hasNext())
        {
          ReadTransmissionData.FreqData f = (ReadTransmissionData.FreqData)freqs.next();
//        System.out.println("; " + f.freq + ", " + f.times + ", " + f.emission + ", " + f.power);
      ...
      while (maps.hasNext())
      {
        ReadTransmissionData.MapData m = (ReadTransmissionData.MapData)maps.next();
//      System.out.println("; " + m.id + ":" + m.top + " " + m.bottom + " - " + m.left + " " + m.right);
      ...
      while (trans.hasNext())
      {
        ReadTransmissionData.TransmissionData t = (ReadTransmissionData.TransmissionData)trans.next();

        if (txData[0].equals(t.starts) && 
            txData[1].equals(t.stops) && 
...
//              ((TransmissionPanel)c).setSelectedTransmissionLine(txId);
                ((TransmissionPanel)c).setSelectedTransmissionLine(t.starts, t.stops);
              }
              break;
            }
          }                                       
