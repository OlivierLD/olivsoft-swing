...
public class ImageTable
	...
	public void setValues() {
		...
		this.table.repaint();
		System.out.println(">> Image table:" + Integer.toString(nbl) + "/ blah image(s)");
				...
				try {
					if (column == 5) {
						if (System.getProperty("verbose", "false").equals("true")) {
							System.out.println("Updating [" + data[row][0] + "] with " + aValue);
						}
						if (aValue != null) {
...
//		}
	...
	public void extractSelectedRows() {
		...
		for (String in : selectedImages) {
			System.out.println(String.format("Extracting %s", in));
//			ImageDBUtils.extractImage(AppContext.getInstance().getConn(), in);
	...
	public void removeSelectedRows() {
		...
		if (resp == 0) {
			...
			for (String in : selectedImages) {
				System.out.println(String.format("Deleting %s", in));
