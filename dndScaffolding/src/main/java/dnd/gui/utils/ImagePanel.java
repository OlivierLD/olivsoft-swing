	...
	private void jbInit() {
		...
		this.jScrollPane1.getViewport().add(new JPanel() {
			public void paintComponent(Graphics gr) {
				...
				gr.drawString("Drag & drop images here to see it", 10, 20);
			}
		}, null);
	}
...
//			AppContext.getInstance().fireRefreshFromDB();
	...
	private void display(final BufferedImage bi) {
		...
		System.out.println(">>> Image was dropped here, do something with it!");
		Throwable t = new Throwable();
//		StackTraceElement[] elements = t.getStackTrace();
//		String message = (elements.length > 1 ? (elements[1].toString() + " : ") : "");
//		System.out.println(message);
		t.printStackTrace();

		// Parse image ? -> Vendor charged you $XX.XX !
