...
import coreutilities.gui.ClockDisplay;
...
public class DesktopFrame
        extends JFrame {
    private final static double DEFAULT_LAT = ((Double) (ParamPanel.getData()[ParamData.DEFAULT_LAT][ParamPanel.PRM_VALUE])).doubleValue();
    private final static double DEFAULT_LON = ((Double) (ParamPanel.getData()[ParamData.DEFAULT_LON][ParamPanel.PRM_VALUE])).doubleValue();
    private ClosestStationFinderThread tideStationFinder = null;
    ...
    private final static SimpleDateFormat SDF_TIDE = new SimpleDateFormat("HH:mm:ss z");
    private final static SimpleDateFormat SDF_DATE_TIDE = new SimpleDateFormat("dd MMM yyyy");
    ...
    private JCheckBoxMenuItem liveWallPaperMenuItem = new JCheckBoxMenuItem();
    ...
    private Boolean liveWallPaper = Boolean.valueOf(false);
    private JInternalFrame specialInternalFrame = new JInternalFrame(); // ForeGround window, transparent
    private List<String> grabbedData = new ArrayList<>();

    double sunAlt = 0, sunZ = 0, moonAlt = 0, moonZ = 0;
    double sunGHA = 0, sunD = 0, moonGHA = 0, moonD = 0;

    private ClockDisplay solarClock;
    private final static Stroke DOTTED = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, new float[]{2}, 0);

    // The globe of the live wallpaper
    private class ChartPanelDesktopPane extends JDesktopPane implements ChartPanelParentInterface {
        ...
        private JCheckBox moonLightCheckBox;
        ...
        private final int TIDE_PANEL_WIDTH = 250;
        private final int TIDE_PANEL_HEIGHT = 150;
        ...
        public ChartPanelDesktopPane() {
            ...
            boolean glossy = ((Boolean) (ParamPanel.getData()[ParamData.LWP_GLOSSY_DISPLAY][ParamPanel.PRM_VALUE])).booleanValue();

            chartPanel.setPositionToolTipEnabled(false);

            chartPanel.setGlobeViewLngOffset(0);
            chartPanel.setGlobeViewRightLeftRotation(23.4);  // Tilt. Will change to the Sun declination
            ...
            headingPanel = new HeadingPanel(false); // false: not glossy
            ...
            headingPanel.setWithColorGradient(glossy);
            headingPanel.setWithCustomColors(true);
            headingPanel.setBgColor(new Color(0f, 0f, 0f, 0f)); // Transparent
            ...
            bspJumbo.setWithGlossyBG(glossy);
            ...
            beaufortJumbo.setWithGlossyBG(glossy);
...
//                                roseCheckBox.getSize().height);
            ...
            sunDayLightCheckBox = new JCheckBox("Sunlight");
...
//                                sunDayLightCheckBox.getSize().height);
            ...
            moonLightCheckBox = new JCheckBox("Moonlight");
            moonLightCheckBox.setHorizontalTextPosition(SwingConstants.LEADING);
            moonLightCheckBox.setForeground(wpFontColor);
            moonLightCheckBox.setSelected(false);
            moonLightCheckBox.setOpaque(false);
            moonLightCheckBox.setVisible(false);
//        moonLightCheckBox.setBounds(this.getWidth() - (moonLightCheckBox.getSize().width + 30),
//                                30 + 5 + HEADING_PANEL_HEIGHT + 5 + bspJumbo.getSize().height,
//                                moonLightCheckBox.getSize().width,
//                                moonLightCheckBox.getSize().height);
            this.add(moonLightCheckBox);
            moonLightCheckBox.addActionListener(new ActionListener() {
                public void actionPerformed(ActionEvent e) {
                    repaint();
                }
            });
...
//      windGaugePanel.setBounds(this.getWidth() - (WINDGAUGE_PANEL_WIDTH + 30), 30, WINDGAUGE_PANEL_WIDTH, WINDGAUGE_PANEL_HEIGHT);
            ...
            windGaugePanel.setGlossy(glossy);
            ...
            if (withTide) {
                ...
                tidePanel.setPreferredSize(new Dimension(TIDE_PANEL_WIDTH, TIDE_PANEL_HEIGHT));
                tidePanel.setSize(new Dimension(TIDE_PANEL_WIDTH, TIDE_PANEL_HEIGHT));
                tidePanel.setVisible(false);
                tidePanel.setBackground(new Color(0f, 0f, 0f, 0f)); // Transparent
                this.add(tidePanel);
            }
            ...
            solarClock.setSize(clockDim);
            solarClock.setVisible(false);
//      solarClock.setBounds(25, 30 + 5 + HEADING_PANEL_HEIGHT, solarClock.getSize().width, solarClock.getSize().height);
            this.add(solarClock);
        }
        ...
        public void specialPaintComponent(Graphics g) // Public around protected
        {
            this.paintComponent(g);
            this.chartPanelPaintComponent(g);
        }
        ...
        public void paintComponent(Graphics gr) {
            ...
            if (liveWallPaper.booleanValue())// aka Live Wallpaper
            {
                double lhaSun = sunGHA + (currentPos == null ? 0 : currentPos.lng);
                ...
                if (false) {
                    try {
                        digiFont = JumboDisplay.tryToLoadFont("ds-digi.ttf", null);
                    } catch (Exception ex) {
                        System.err.println("Loading font:" + ex.getMessage());
                    }
                } else {
                    try {
...
//            System.out.println("Loaded Font [" + digiFont.toString() + "]");
                        ...
                        try {
                            digiFont = new Font("Courier New", 18, Font.BOLD);
                        } catch (Exception ex2) {
                            System.err.println("Loading font (courier new):" + ex2.getMessage());
                        }
                    }
                }
                if (digiFont == null) {
                ...
                synchronized (grabbedData) {
                    List<String> data = new ArrayList<>(grabbedData); // Clone
                    for (String s : data) {
                ...
                // Solar Clock just below the last one
                //  gr.drawString("Solar Clock here soon...", 25, y);
                //  solarClock.setBounds(25, y, solarClock.getSize().width, solarClock.getSize().height);
                solarClock.setBounds(this.getWidth() - 2 * (solarClock.getSize().width + 30), 40 + HEADING_PANEL_HEIGHT, solarClock.getSize().width, solarClock.getSize().height);
                solarClock.setVisible(true);
                ...
                try {
                    synchronized (ndc) {
                        Angle360 twdAngle = (Angle360) ndc.get(NMEADataCache.TWD, true);
                        if (twdAngle != null) {
                            twd = (int) twdAngle.getDoubleValue();
                        }
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                ...
                try {
                    synchronized (ndc) {
                        Angle360 hdgAngle = (Angle360) ndc.get(NMEADataCache.HDG_TRUE, true);
                        if (hdgAngle != null) {
                            hdg = (int) hdgAngle.getDoubleValue();
                        }
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                ...
                if (displayRose && roseCheckBox.isSelected()) {
                    ...
                    for (int i = 0; i < 360; i += 5) {
                        x1 = (dim.width / 2) + (int) ((radius - (i % 45 == 0 ? 20 : 10)) * Math.cos(Math.toRadians(i)));
                        y1 = (dim.height / 2) + (int) ((radius - (i % 45 == 0 ? 20 : 10)) * Math.sin(Math.toRadians(i)));
                        x2 = (dim.width / 2) + (int) ((radius) * Math.cos(Math.toRadians(i)));
                        y2 = (dim.height / 2) + (int) ((radius) * Math.sin(Math.toRadians(i)));
...
//          radius *= 0.95;
                    ...
                    if (twd != -1) {
                        // TWD
                        x1 = (dim.width / 2) + (int) ((radius * 0.75) * Math.cos(Math.toRadians(twd + 90)));
                        y1 = (dim.height / 2) + (int) ((radius * 0.75) * Math.sin(Math.toRadians(twd + 90)));
                        x2 = (dim.width / 2) - (int) ((radius * 0.75) * Math.cos(Math.toRadians(twd + 90)));
                        y2 = (dim.height / 2) - (int) ((radius * 0.75) * Math.sin(Math.toRadians(twd + 90)));
                ...
                // Position on a globe?
                synchronized (ndc) {
                    currentPos = (GeoPos) ndc.get(NMEADataCache.POSITION, false);
                }
                if (currentPos == null) // No data from the GPS, take it from the preferences
                {
                    currentPos = new GeoPos(DEFAULT_LAT, DEFAULT_LON);
                }
                if (currentPos != null) {
...
//          chartPanel.setGlobeViewRightLeftRotation(23.0);  // Tilt
                    ...
                    if (hdg != -1) {
                        ...
                        //  headingPanel.setValue(hdg);
                    }
                    double bsp = -1d;
                    try {
                        synchronized (ndc) {
                            Speed bspSpeed = (Speed) ndc.get(NMEADataCache.BSP, true);
                            if (bspSpeed != null)
                                bsp = bspSpeed.getDoubleValue();
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                    ...
                    try {
                        synchronized (ndc) {
                            Speed twsSpeed = (Speed) ndc.get(NMEADataCache.TWS, true);
                            if (twsSpeed != null)
                                tws = twsSpeed.getDoubleValue();
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                    beaufortJumbo.setVisible(tws > -1);
                    if (tws > -1) {
                        beaufortJumbo.setBounds(this.getWidth() - (2 * beaufortJumbo.getSize().width + 30) - 5,
                                currentYPos,
                                beaufortJumbo.getSize().width,
                                beaufortJumbo.getSize().height);
                        gr.drawRoundRect(this.getWidth() - (2 * beaufortJumbo.getSize().width + 30) - 5,
                                currentYPos,
                                beaufortJumbo.getSize().width,
                                beaufortJumbo.getSize().height, 5, 5);
                    }
                    // Display, below
                    ...
                    sunDayLightCheckBox.setBounds(this.getWidth() - (sunDayLightCheckBox.getSize().width + moonLightCheckBox.getSize().width + 30),
                            currentYPos,
                            sunDayLightCheckBox.getPreferredSize().width,
                            sunDayLightCheckBox.getPreferredSize().height);
                    moonLightCheckBox.setVisible(true);
                    moonLightCheckBox.setEnabled(globeCheckBox.isSelected());
                    //  currentYPos += (5 + roseCheckBox.getPreferredSize().height);
                    moonLightCheckBox.setBounds(this.getWidth() - (moonLightCheckBox.getSize().width + 30),
                            currentYPos,
                            moonLightCheckBox.getPreferredSize().width,
                            moonLightCheckBox.getPreferredSize().height);
...
//                                 MARQUEE_PANEL_HEIGHT, 10, 10);
                            ...
                            // Tide? Find nearest station
                            if (tideStationFinder == null) {
                                currentPos = null;
                                synchronized (ndc) {
                                    currentPos = (GeoPos) ndc.get(NMEADataCache.POSITION, false);
                                }
                                ...
                                tideStationFinder = new ClosestStationFinderThread(currentPos.lat, currentPos.lng);
                                tideStationFinder.start();
                            }
                            if (closestTideStation != null) {
//                System.out.println("Tide Station: " + closestTideStation.getFullName());
                                UTCDate utcDate = null;
                                synchronized (ndc) {
                                    utcDate = (UTCDate) ndc.get(NMEADataCache.GPS_DATE_TIME, true);
                                    if (utcDate == null)
                                        utcDate = new UTCDate(new Date()); // TimeUtil.getGMT());
                                }
                                Calendar now = GregorianCalendar.getInstance();
                                now.setTimeZone(TimeZone.getTimeZone(closestTideStation.getTimeZone()));
                                SDF_TIDE.setTimeZone(TimeZone.getTimeZone(closestTideStation.getTimeZone()));
                                SDF_DATE_TIDE.setTimeZone(TimeZone.getTimeZone(closestTideStation.getTimeZone()));
                                if (utcDate != null)
                                    now.setTime(utcDate.getValue());
                                try {
                                    List<TimedValue> timeAL = calculateTideTable(closestTideStation, now, constSpeed);
                                    double wh = TideUtilities.getWaterHeight(closestTideStation, constSpeed, now);

//                    for (TimedValue tv : timeAL)
//                      System.out.println(tv.getType() + " " + SDF_TIDE.format(tv.getCalendar().getTime()) +
//                                         " : " + TideUtilities.DF22PLUS.format(tv.getValue()) + " " +
//                                         closestTideStation.getDisplayUnit());
//                    System.out.println(now.getTime().toString() + " : " + TideUtilities.DF22PLUS.format(wh) + " " + closestTideStation.getDisplayUnit());

                                    List<String> tideTable = new ArrayList<>();
                                    tideTable.add("Tide at " + closestTideStation.getFullName());
                                    tideTable.add("On " + SDF_DATE_TIDE.format(now.getTime()));
                                    for (TimedValue tv : timeAL)
                                        tideTable.add(tv.getType() + " " + SDF_TIDE.format(tv.getCalendar().getTime()) +
                                                " : " + TideUtilities.DF22PLUS.format(tv.getValue()) + " " +
                                                closestTideStation.getDisplayUnit());
                                    tideTable.add(SDF_TIDE.format(now.getTime()) + " : " + TideUtilities.DF22PLUS.format(wh) + " " + closestTideStation.getDisplayUnit());
                                    Font origFont = gr.getFont();
                                    gr.setFont(origFont.deriveFont(12f));
                                    int maxWidth = 0;
                                    for (String s : tideTable) {
                                        int strWidth = gr.getFontMetrics(gr.getFont()).stringWidth(s);
                                        maxWidth = Math.max(maxWidth, strWidth);
                                    }
                                    maxWidth += 18;
                                    int x = this.getWidth() - maxWidth; // other side, x = 25;
                                    currentYPos += 15;
                                    for (String s : tideTable) {
                                        gr.drawString(s, x, currentYPos);
                                        currentYPos += (gr.getFont().getSize() + 2);
                                    }
                                    gr.setFont(origFont);
                                } catch (Exception e) {
                                    e.printStackTrace();
                                }
                                tidePanel.setBounds(this.getWidth() - (TIDE_PANEL_WIDTH + 30),
//                                   currentYPos,                                    // Next in line...
                                        this.getHeight() - (TIDE_PANEL_HEIGHT + 30), // Bottom of the wallpaper
                                        TIDE_PANEL_WIDTH,
                                        TIDE_PANEL_HEIGHT);
                                try {
                                    List<Double> tidedata = calculateTideData(closestTideStation, now, constSpeed);
                                    tidePanel.setData(tidedata);
                                    tidePanel.setCal(now);
                                } catch (Exception ex) {
                                    ex.printStackTrace();
                                }
                                tidePanel.setVisible(true);
                            }
                        }
                    }
                }
                ...
                sunDayLightCheckBox.setVisible(false);
                moonLightCheckBox.setVisible(false);
                marqueeCheckBox.setVisible(false);
                sunAzimuthCheckBox.setVisible(false);
                moonAzimuthCheckBox.setVisible(false);
                headingPanel.setVisible(false);
                windGaugePanel.setVisible(false);
...
//      System.out.println(">>> DEBUG >>> Bottom of the paintComponent");
        ...
        public void chartPanelPaintComponent(Graphics gr) {
            if (liveWallPaper.booleanValue() && currentPos != null) {
                ...
                if (DayLight.isVisible(chartPanel, ls, gs)) {
                    gr.setColor(Color.red);
                    gr.fillOval(gp.x - 3, gp.y - 3, 6, 6);
                    chartPanel.postit(gr, "Position", gp.x + 2, gp.y - 2, Color.yellow);
                }
                // Sun pos
                double gSun = 0.0;
                if (sunGHA < 180)
                ...
                if (sunGHA >= 180)
                    gSun = 360 - sunGHA;
                if (DayLight.isVisible(chartPanel, sunD, gSun)) {
                    gp = chartPanel.getPanelPoint(currentPos.lat, currentPos.lng);
                    Point sun = chartPanel.getPanelPoint(sunD, gSun);
                    gr.setColor(Color.orange);
                    gr.fillOval(sun.x - 3, sun.y - 3, 6, 6);
                    Stroke origin = g2d.getStroke();
                    g2d.setStroke(DOTTED);
                    gr.drawLine(gp.x, gp.y, sun.x, sun.y);
                    g2d.setStroke(origin);
                    chartPanel.postit(gr, "Sun", sun.x + 2, sun.y - 2, Color.yellow);
                    //  Point sunInTheSky = new Point(sun.x + (sun.x - gp.x), sun.y + (sun.y - gp.y));
                    //  gr.drawLine(sun.x, sun.y, sunInTheSky.x, sunInTheSky.y);
                    //  gr.fillOval(sunInTheSky.x - 5, sunInTheSky.y - 5, 10, 10);

                    // GC between position and sun. Position in the center, GC looks like a straight line.
//            GreatCircle pos2sun = new GreatCircle();
//            pos2sun.setStartInDegrees(new GeoPoint(currentPos.lat, currentPos.lng));
//            pos2sun.setArrivalInDegrees(new GeoPoint(sunD, gSun));
//            pos2sun.calculateGreatCircle(10);
//            Vector<GreatCircleWayPoint> route = pos2sun.getRoute();
//            Point prev = gp;
//            for (GreatCircleWayPoint gcwp : route) {
//              Point p = chartPanel.getPanelPoint(Math.toDegrees(gcwp.getPoint().getL()), Math.toDegrees(gcwp.getPoint().getG()));
//              gr.drawLine(prev.x, prev.y, p.x, p.y);
//              gr.fillOval(p.x - 1, p.y - 1, 2, 2);
//              prev = p;
//            }
                }
                ...
                if (moonGHA >= 180)
                    gMoon = 360 - moonGHA;
                if (DayLight.isVisible(chartPanel, moonD, gMoon)) {
                    gr.setColor(Color.white);
                    gp = chartPanel.getPanelPoint(currentPos.lat, currentPos.lng);
                    // GC between position and moon
                    Point moon = chartPanel.getPanelPoint(moonD, gMoon);
                    Stroke origin = g2d.getStroke();
                    g2d.setStroke(DOTTED);
                    gr.drawLine(gp.x, gp.y, moon.x, moon.y);
                    g2d.setStroke(origin);
                    gr.fillOval(moon.x - 3, moon.y - 3, 6, 6);
                    chartPanel.postit(gr, "Moon", moon.x + 2, moon.y - 2, Color.yellow);
//            GreatCircle pos2sun = new GreatCircle();
//            pos2sun.setStartInDegrees(new GeoPoint(currentPos.lat, currentPos.lng));
//            pos2sun.setArrivalInDegrees(new GeoPoint(moonD, gMoon));
//            pos2sun.calculateGreatCircle(10);
//            Vector<GreatCircleWayPoint> route = pos2sun.getRoute();
//            Point prev = gp;
//            for (GreatCircleWayPoint gcwp : route) {
...
//            }
                ...
                if (sunDayLightCheckBox.isSelected()) {
                    DayLight.displayDayLight(sunGHA, sunD, chartPanel, gr, currPos);
                }

                if (moonLightCheckBox.isSelected()) {
                    DayLight.displayDayLight(moonGHA, moonD, chartPanel, gr, currPos);
                }
            } else
                chartPanel.setVisible(false);
        }
    ...
    private static List<BackgroundWindow> bgwal = new ArrayList<>(1);
...
//  NMEA_EOS = new String(new char[] {0x0A, 0x0D}); //(System.getProperty("os.name").contains("Windows")?NMEAParser.STANDARD_NMEA_EOS:NMEAParser.LINUX_NMEA_EOS);
    ...
    //private final static int RMI_NMEA_SERVER      = 10; // Unused, to be removed or reused
...
//      System.out.println("DataString:[" + dataString + "]");
            ...
            if (idxAWS > -1) {
                try {
                    ...
                    System.err.println("AWS:" + nfe.getLocalizedMessage());
                }
            }
            if (idxAWA > -1) {
                try {
                    ...
                    System.err.println("AWA:" + nfe.getLocalizedMessage());
                }
            }
...
//  this.setSize(new Dimension(800, 600));
        ...
        try {
            Class.forName("generatelocator.GenInternalFrame");
...
//  backGroundNMEAServer.setSelected(false);
        ...
        liveWallPaperMenuItem.setText("Show live wallpaper"); // "Show foreground data");
//  foregroundDataMenuItem.setToolTipText("<html>Shows current data <br><b>on top</b> of all windows.</html>");
        liveWallPaperMenuItem.setSelected(liveWallPaper.booleanValue());

        chartLibMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(CHARTLIB);
            }
        });
        ...
        replayNmeaMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(REPLAY_NMEA);
            }
        });
        logToGPXMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(LOG_TO_GPX);
            }
        });
        logAnalyzerMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(LOG_ANALYZER);
            }
        });

        starFinderMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(STARFINDER);
            }
        });
        d2102StarFinderMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(STARFINDER_2012D);
            }
        });
        sailFaxMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(SAILFAX);
            }
        });
        spotMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(SPOT);
            }
        });
        lunarMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(LUNAR);
            }
        });
        srMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(SR);
            }
        });
        realTimeAlmanacMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(REAL_TIME_ALMANAC);
            }
        });
        almanacMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(ALMANAC);
            }
        });
        locatorMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(LOCATOR);
            }
        });
        tideMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(TIDES);
            }
        });

        backGroundNMEARead.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                appRequest_ActionPerformed(READ_REBROADCAST);
            }
        });
//  backGroundNMEAServer.addActionListener(new ActionListener()  { public void actionPerformed( ActionEvent ae ) { appRequest_ActionPerformed(RMI_NMEA_SERVER); } } );
        ...
        liveWallPaperMenuItem.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                liveWallPaper = Boolean.valueOf(liveWallPaperMenuItem.isSelected());
//      specialInternalFrame.setVisible(foregroundData.booleanValue());
        ...
        menuHelpRebroadcast.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                helpRebroadcast_ActionPerformed(ae);
            }
        });
...
//    nmeaMenuItem.add(backGroundNMEAServer);
        ...
        menuToolsFullScreen.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent ae) {
                fullScreen();
            }
        });
...
//  menuToolsFullScreen.setEnabled(false);
        ...
        menuTools.add(liveWallPaperMenuItem);
        menuTools.add(new JSeparator());
            ...
            protected void paintComponent(Graphics g) {
                if (liveWallPaper.booleanValue()) {
                    ...
                    if (false) {
                        ...
                        try {
                            digiFont = new Font("Source Code Pro", 10, Font.PLAIN);
                        } catch (Exception ex) {
                            try {
                                digiFont = new Font("Courier New", 10, Font.PLAIN);
                            } catch (Exception ex2) {
                                System.err.println("Loading source code pro, courier new:" + ex2.getMessage());
                            }
                        }
                    }
                    if (digiFont != null) {
...
//  desktop.add(specialInternalFrame);
        ...
        ChartLibContext.getInstance().addChartLibListener(new ChartLibListener() {
            public void internalFrameClosed() {
                try {
                    SQLUtil.shutdown(chartConnection);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
...
//        try { SQLUtil.shutdown(logConnection); } catch (Exception ex) { ex.printStackTrace(); }
        ...
        if (((Boolean) (ParamPanel.getData()[ParamData.NMEA_AT_STARTUP][ParamPanel.PRM_VALUE])).booleanValue())
            startReadingNMEA();

        if (((Boolean) (ParamPanel.getData()[ParamData.LWP_AT_STARTUP][ParamPanel.PRM_VALUE])).booleanValue()) {
            liveWallPaper = Boolean.TRUE;
            liveWallPaperMenuItem.setSelected(liveWallPaper.booleanValue());
            startDataGrabber();
        }
    }

    private void startReadingNMEA() {
        backGroundNMEARead.setSelected(true);
        appRequest_ActionPerformed(READ_REBROADCAST); // Forwarding event
    }
...
//  System.out.println("Setting BG Image to " + imgURL.toString());
    ...
    public boolean isNMEAConsoleRunning() {
        return !nmeaConsoleMenuItem.isEnabled();
    }

    public boolean isStarFinderRunning() {
        return !starFinderMenuItem.isEnabled();
    }

    public boolean isLocatorRunning() {
        return !locatorMenuItem.isEnabled();
    }

    public boolean isChartLibRunning() {
        return !chartLibMenuItem.isEnabled();
    }

    public boolean isSailFaxRunning() {
        return !sailFaxMenuItem.isEnabled();
    }

    public boolean isSRRunning() {
        return !srMenuItem.isEnabled();
    }

    public boolean isAPRunning() {
        return !almanacMenuItem.isEnabled();
    }

    public boolean isTideRunning() {
        return !tideMenuItem.isEnabled();
    }
    ...
    private void appRequest_ActionPerformed(int app) {
        ...
        switch (app) {
            ...
            case CHARTLIB:
                ...
                try {
                    try {
                        chartConnection = SQLUtil.getConnection(dbLocation, "CHART", "chart", "chart");
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
...
//            if (tides == null)
                        {
                            ...
                            try {
                                ...
                                System.err.println("Tide Thread, ioe:" + ioe.getLocalizedMessage());
                            } catch (NumberFormatException nfe) {
                                System.err.println("Tide Thread, format:" + nfe.getLocalizedMessage());
                            }
                            desktop.add(tides);
                        }
    ...
    private void centerFrame(Dimension masterDim, JInternalFrame fr) {
        ...
        if (fr instanceof NMEAInternalFrame) {
            ...
            if (consolePos.exists()) {
                ...
                synchronized (parser) {
                    try {
                        parser.parse(consolePos.toURI().toURL());
                    } catch (Exception ex) {
                        ex.printStackTrace();
                        dataOk = false;
                    }
    ...
    private void startDataGrabber() {
        Thread dataGrabber = new Thread("Data Grabber") {
            public void run() {
                ...
                try {
                    while (liveWallPaper.booleanValue()) {
//            System.out.println((">>> DEBUG >>> DataGrabber - top of the loop"));
                        synchronized (grabbedData) {
                            ...
                            try {
                                synchronized (ndc) {
                                    GeoPos gpsPos = (GeoPos) ndc.get(NMEADataCache.POSITION, true);
                                    if (gpsPos == null)
                                        gpsPos = new GeoPos(DEFAULT_LAT, DEFAULT_LON);
                                    str += ("L:" + lpad(GeomUtil.decToSex((gpsPos).lat, GeomUtil.SWING, GeomUtil.NS), " ", 12));
                                }
                            } catch (Exception ex) {
                                System.err.println("LWP 1 :" + ex.getLocalizedMessage());
                                str += "-";
                            }
                            grabbedData.add(str);
                            str = "";
                            ...
                            try {
                                synchronized (ndc) {
                                    str += Integer.toString((int) ((Angle360) ndc.get(NMEADataCache.COG, true)).getValue());
                                }
                            } catch (Exception ex) {
//                System.err.println("COG in Cache:" + ex.getLocalizedMessage());
                            ...
                            try {
                                synchronized (ndc) {
                                    str += DF22.format(((Speed) ndc.get(NMEADataCache.SOG, true)).getDoubleValue());
                                }
                            } catch (Exception ex) {
                                //    System.err.println("SOG in Cache");
                                hsString += "-";
                            } // TODO A Format
                            str += " kts";
                            grabbedData.add(str);
                            ...
                            try {
                                try {
                                    ...
                                    System.err.println("GPS Time:" + ignore.getLocalizedMessage());
                                }
                                long time = 0L;
                                try {
                                    synchronized (ndc) {
                                        UTCTime utcTime = (UTCTime) ndc.get(NMEADataCache.GPS_TIME, true);
                                        if (utcTime == null)
                                            utcTime = new UTCTime(new Date()); // TimeUtil.getGMT());
                                        time = utcTime.getValue().getTime();
                                    }
                                } catch (NullPointerException npe) {
                                    System.err.println("NPE for GPS Time...");
                                ...
                                if (solarClock != null && time != 0L)
                                    solarClock.setValue(time);

                                grabbedData.add(str);
                                str = "Solar Offset (~G):" + decimalHoursToHMS(offset / (3600 * 1000));
                                grabbedData.add(str);
                                if (g != 0) {
                                    ...
                                    try {
                                        ...
                                        System.err.println("Sun Max Alt:" + ex.getLocalizedMessage());
                                        str += "-";
                                        grabbedData.add(str);
                                    }
                                }
                            ...
                            if (true) // TODO Option
                            {
                                ...
                                if (utcDate == null && gpsPos == null) // No NMEA data
                                {
                                    utcDate = new UTCDate(new Date()); //TimeUtil.getGMT());
                                    gpsPos = new GeoPos(DEFAULT_LAT, DEFAULT_LON);
                                }
                                if (utcDate != null && gpsPos != null) {
                                    ...
                                    double lhaSun = sunGHA + g;
                                    while (lhaSun < 0) lhaSun += 360;
                                    while (lhaSun > 360) lhaSun -= 360;
                                    double lhaMoon = moonGHA + g;
                                    while (lhaMoon < 0) lhaMoon += 360;
                                    while (lhaMoon > 360) lhaMoon -= 360;

                                    grabbedData.add("Sun GHA: " + GeomUtil.decToSex(ghaSun, GeomUtil.SWING, GeomUtil.NONE) +
                                            ", Dec: " + GeomUtil.decToSex(decSun, GeomUtil.SWING, GeomUtil.NS) +
                                            ", LHA: " + GeomUtil.decToSex(lhaSun, GeomUtil.SWING, GeomUtil.NONE));
                                    ...
                                    grabbedData.add("Moon GHA: " + GeomUtil.decToSex(ghaMoon, GeomUtil.SWING, GeomUtil.NONE) +
                                            ", Dec: " + GeomUtil.decToSex(decMoon, GeomUtil.SWING, GeomUtil.NS) +
                                            ", LHA: " + GeomUtil.decToSex(lhaMoon, GeomUtil.SWING, GeomUtil.NONE));
                            ...
                            double bsp = -1d;
                            int hdg = -1;
                            double bl = -1d; // Big Log
                            double sl = -1d; // Small log
                            double depth = -1d;
                            double temp = -1d;
                            ...
                            synchronized (ndc) {
                                try {
                                    aws = ((Speed) ndc.get(NMEADataCache.AWS, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    awa = (int) ((Angle180) ndc.get(NMEADataCache.AWA, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    bsp = ((Speed) ndc.get(NMEADataCache.BSP, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    hdg = (int) ((Angle360) ndc.get(NMEADataCache.HDG_TRUE, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }

                                try {
                                    bl = ((Distance) ndc.get(NMEADataCache.LOG, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    sl = ((Distance) ndc.get(NMEADataCache.DAILY_LOG, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    depth = ((Depth) ndc.get(NMEADataCache.DBT, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    temp = ((Temperature) ndc.get(NMEADataCache.WATER_TEMP, true)).getValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    atemp = ((Temperature) ndc.get(NMEADataCache.AIR_TEMP, true)).getValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    bpress = ((Pressure) ndc.get(NMEADataCache.BARO_PRESS, true)).getValue();
                                } catch (Exception ex) {
                                }

                                try {
                                    tws = ((Speed) ndc.get(NMEADataCache.TWS, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
                                try {
                                    twd = (int) ((Angle360) ndc.get(NMEADataCache.TWD, true)).getDoubleValue();
                                } catch (Exception ex) {
                                }
...
//                        System.out.println("1 - Nb entry(ies) in Calculated Current Map:" + keys.size());
                            ...
                            if (aws != -1)
                                grabbedData.add("AWS:" + (Math.abs(aws) == Double.MAX_VALUE ? " -" : DF22.format(aws)) + " kts, AWA:" + Integer.toString(awa) + degSymbol);
                            int beaufort = WindUtils.getBeaufort(tws);
                            if (beaufort != -1)
                                grabbedData.add("Wind: F" + beaufort + " - " + WindUtils.getRoseDir(twd));
                            if (bsp != -1)
                                grabbedData.add("BSP:" + (Math.abs(bsp) == Double.MAX_VALUE ? " -" : (DF22.format(bsp) + " kts")));
                            if (hdg != -1)
...
//              System.out.println("... >>> DEBUG >>> Desktop.repaint");
                            ...
                            // Generate image from the background? TODO Option
                            if (false) {
                                SwingUtilities.invokeAndWait(new Runnable() {
                                    public void run() {
                                        ...
                                        ((ChartPanelDesktopPane) desktop).specialPaintComponent((Graphics) g2d);
                                        // Graphics context no longer needed so dispose it
                                        g2d.dispose();

                                        try {
                                            String ext = "jpg"; // "bmp";
                                            OutputStream os = new FileOutputStream("background." + ext);
...
//                }
    ...
    private static List<TimedValue> calculateTideTable(TideStation ts, Calendar now, List<Coefficient> constSpeed)
            throws Exception {
    ...
    private static List<Double> calculateTideData(TideStation ts, Calendar now, List<Coefficient> constSpeed)
            throws Exception {
...
//    System.out.println("End of timer");
    ...
    private class ClosestStationFinderThread extends Thread {
        private GeoPoint origin = null;

        public ClosestStationFinderThread(double lat, double lon) {
        ...
        public void run() {
            ...
            // System.out.println("TideStation list OK");
            ...
            if (tideStations != null) {
                ...
                List<TideInternalFrame.StationDistance> stationMap = new ArrayList<>();
                // Populate
                for (TideStation tideStation : tideStations) {
                    if (tideStation.isTideStation()) // Tide Stations only
                    {
                ...
                for (TideInternalFrame.StationDistance tideStation : stationMap) {
                    System.out.println(tideStation.getStationName() + " at " + tideStation.getDistance() + " nm");
                    if (tideStation.getDistance() < 100) { // Less than 100 nm.
                        try {
                            int year = new GregorianCalendar().get(Calendar.YEAR); // Questionable. What if replay?
                            closestTideStation = BackEndTideComputer.findTideStation(tideStation.getStationName(), year);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    }
...
//          posString += ("L:" + GeomUtil.decToSex(NMEACache.getInstance().getLat(), GeomUtil.SWING, GeomUtil.NS));
                        synchronized (ndc) {
                            posString += ("L:" + GeomUtil.decToSex(((GeoPos) ndc.get(NMEADataCache.POSITION, true)).lat, GeomUtil.SWING, GeomUtil.NS));
                        }
                    } catch (Exception ex) {
                        System.err.println("L:" + ex.getLocalizedMessage());
                        posString += "-";
                    }
                    posString += "\n";
                    try {
//          g = NMEACache.getInstance().getLng();
                        ...
                        System.out.println("G:" + ex.getLocalizedMessage());
                        posString += "-";
                    }
                    hsString = "COG:";
                    try {
//          hsString += Integer.toString(((int)NMEACache.getInstance().getCog()));
                        synchronized (ndc) {
                            hsString += Integer.toString((int) ((Angle360) ndc.get(NMEADataCache.COG, true)).getValue());
                        }
                    } catch (Exception ex) {
//          System.err.println("COG in Cache:" + ex.getLocalizedMessage());
                        hsString += "-";
...
//          hsString += (NMEACache.getInstance().getSog());
                        synchronized (ndc) {
                            hsString += DF22.format(((Speed) ndc.get(NMEADataCache.SOG, true)).getDoubleValue());
                        }
                    } catch (Exception ex) {
                        //    System.err.println("SOG in Cache");
                        hsString += "-";
...
//            gpsTimeString = "GPS Time:" + sdf3.format(((UTCDate)NMEAContext.getInstance().getCache().get(NMEADataCache.GPS_DATE_TIME, true)).getValue());
                            synchronized (ndc) {
                                UTCDate utc = (UTCDate) ndc.get(NMEADataCache.GPS_DATE_TIME, true);
                                if (utc == null) {
                                    Date ut = TimeUtil.getGMT();
                                    utc = new UTCDate(ut);
                                }
                                gpsTimeString = SDF.format(utc.getValue());
                            }
                        } catch (Exception ignore) {
                            System.err.println("GPS DateTime:" + ignore.getLocalizedMessage());
                        }
                        long time = 0L;
                        try {
//            time = NMEACache.getInstance().getGpsDate().getTime();
                            synchronized (ndc) {
                                UTCTime utcTime = (UTCTime) ndc.get(NMEADataCache.GPS_TIME, true);
                                if (utcTime == null)
                                    utcTime = new UTCTime(TimeUtil.getGMT());
                                time = utcTime.getValue().getTime();
                            }
                        } catch (NullPointerException npe) {
                            System.err.println("NPE for GPS Time...");
    ...
    private class HTTPNMEAReaderThread extends Thread {
        ...
        public void run() {
                    ...
                    getBGWinByTitle(AW_BG_WINDOW_TITLE).isDisplayBGWindow()) // TODO REAL_TIMNE_ALMANAC
            {
                String allNMEAData = "";
                try {
                    allNMEAData = HTTPClient.getContent(nmeaServerURL);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                DOMParser parser = DesktopContext.getInstance().getParser();
                synchronized (parser) {
                    try {
                        ...
                        posString = "";
                        try {
                            posString += ("L:" + GeomUtil.decToSex(Double.parseDouble(dataDoc.selectNodes("/data/lat").item(0).getFirstChild().getNodeValue()), GeomUtil.SWING, GeomUtil.NS));
                        } catch (Exception ex) {
                            posString += "-";
                        }
                        posString += "\n";
                        ...
                        hsString = "COG:";
                        try {
                            hsString += Integer.toString(((int) Double.parseDouble(dataDoc.selectNodes("/data/cog").item(0).getFirstChild().getNodeValue())));
                        } catch (Exception ex) {
                            hsString += "-";
                        }
                        hsString += " \272\nSOG:";
                        try {
                            hsString += (Double.parseDouble(dataDoc.selectNodes("/data/sog").item(0).getFirstChild().getNodeValue()));
                        } catch (Exception ex) {
                            hsString += "-";
                        }
                        hsString += " kts";
            ...
            if (event.getActionCommand().equals(REBROADCAST)) {
                ...
                if (resp == JOptionPane.OK_OPTION) {
                    if (rebroadcastPanel.isHTTPSelected() && HTTPPort == -1) {
                        ...
                        // Stop the server
                        try {
                            HTTPClient.getContent("http://localhost:" + port + "/exit");
                        } catch (Exception ex) {
                        }
                        HTTPPort = -1;
                        html5Console.setEnabled(false);
...
//          System.out.println("Creating TCP writer on " + rebroadcastPanel.tcpHost() + ":" + TCPPort);
                        ...
                        try {
                            ...
                            System.err.println("TCPWriter:" + ex.getLocalizedMessage());
                        }
                    }
...
//    label.setBackground(Color.yellow);
        ...
        private String composeStringFromCache() {
            double aws = 0d;
            try {
                aws = ((Speed) NMEAContext.getInstance().getCache().get(NMEADataCache.AWS, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            int awa = 0;
            try {
                awa = (int) ((Angle180) NMEAContext.getInstance().getCache().get(NMEADataCache.AWA, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double bsp = 0d;
            try {
                bsp = ((Speed) NMEAContext.getInstance().getCache().get(NMEADataCache.BSP, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            int hdg = 0;
            try {
                hdg = (int) ((Angle360) NMEAContext.getInstance().getCache().get(NMEADataCache.HDG_TRUE, true)).getDoubleValue();
            } catch (Exception ex) {
            }

            double bl = 0d;
            try {
                bl = ((Distance) NMEAContext.getInstance().getCache().get(NMEADataCache.LOG, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double sl = 0d;
            try {
                sl = ((Distance) NMEAContext.getInstance().getCache().get(NMEADataCache.DAILY_LOG, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double depth = 0d;
            try {
                depth = ((Depth) NMEAContext.getInstance().getCache().get(NMEADataCache.DBT, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double temp = 0d;
            try {
                temp = ((Temperature) NMEAContext.getInstance().getCache().get(NMEADataCache.WATER_TEMP, true)).getValue();
            } catch (Exception ex) {
            }
            int twd = 0;
            try {
                twd = (int) ((Angle360) NMEAContext.getInstance().getCache().get(NMEADataCache.TWD, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double tws = 0d;
            try {
                tws = ((Speed) NMEAContext.getInstance().getCache().get(NMEADataCache.TWS, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            double twa = 0d;
            try {
                twa = ((Angle180) NMEAContext.getInstance().getCache().get(NMEADataCache.TWA, true)).getValue();
            } catch (Exception ignore) {
            }
            int cdr = 0;
...
//    try { csp = ((Speed)NMEAContext.getInstance().getCache().get(NMEADataCache.CSP, true)).getDoubleValue(); } catch (Exception ex) {}
            ...
            float bat = 0;
            try {
                bat = ((Float) NMEAContext.getInstance().getCache().get(NMEADataCache.BATTERY, true)).floatValue();
            } catch (Exception ignore) {
            }
            double xte = 0;
            try {
                xte = ((Distance) NMEAContext.getInstance().getCache().get(NMEADataCache.XTE, true)).getValue();
            } catch (Exception ignore) {
            }
            String nwp = "";
            ...
            try {
                airtemp = ((Temperature) NMEAContext.getInstance().getCache().get(NMEADataCache.AIR_TEMP, true)).getValue();
            } catch (Exception ignore) {
            }
            double prmsl = 0d;
            try {
                prmsl = ((Pressure) NMEAContext.getInstance().getCache().get(NMEADataCache.BARO_PRESS, true)).getValue();
            } catch (Exception ignore) {
            }
            ...
            int cog = 0;
            try {
                sog = ((Speed) NMEAContext.getInstance().getCache().get(NMEADataCache.SOG, true)).getDoubleValue();
            } catch (Exception ex) {
            }
            try {
                cog = (int) ((Angle360) NMEAContext.getInstance().getCache().get(NMEADataCache.COG, true)).getDoubleValue();
            } catch (Exception ex) {
            }

            // A preference for the data to display in the marquee
            ...
            for (String id : da) {
                if ("BSP".equals(id))
                    ...
                    str += ("NWP:" + lpad(nwp, " ", 6) + "\n");
                else if ("COG".equals(id) && sog != -1)
                    str += ("COG:" + lpad(DIR_FMT.format(cog), " ", 5) + "\n");
                else if ("SOG".equals(id) && sog != -1)
                    str += ("SOG:" + lpad(SPEED_FMT.format(sog), " ", 8) + "\n");
            }
//    System.out.println("Composed " + str);
    ...
    private class TidePanel
        ...
        public void paintComponent(Graphics gr) {
            ...
            gr.drawRoundRect(1, 1, this.getWidth() - 3, this.getHeight() - 3, 20, 20);
            ((Graphics2D) gr).setStroke(orig);
            // Draw tide curve here
            if (this.data != null) {
                ...
                int vertPad = 5;
                float stepX = (float) this.getWidth() / (float) size;
                float yProp = (float) (this.getHeight() - (2 * vertPad)) / (float) (max - min);
                Point prevPoint = null;
                for (int i = 0; i < size; i++) {
                    double wh = this.data.get(i);
                    Point p = new Point(Math.round(i * stepX), (this.getHeight() - vertPad) - (int) Math.round((wh - min) * yProp));
                    if (prevPoint != null) {
                        gr.drawLine(prevPoint.x, prevPoint.y, p.x, p.y);
                    }
                    prevPoint = p;
                }
                // Current date-time, vertical line.
                if (this.cal != null) {
                    int h = cal.get(Calendar.HOUR_OF_DAY);
                    int m = cal.get(Calendar.MINUTE);
                    int s = cal.get(Calendar.SECOND);
                    int nbSec = s + (60 * m) + (3600 * h);
                    float inDay = (float) nbSec / (float) (3600 * 24);
                    gr.setColor(Color.white);
                    int abs = Math.round((float) this.getWidth() * inDay);
                    gr.drawLine(abs, 0, abs, this.getHeight());
                }
            }
        }
    }
