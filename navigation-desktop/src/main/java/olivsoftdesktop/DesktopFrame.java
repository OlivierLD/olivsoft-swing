...
//      solarClock.setBounds(25, 30 + 5 + HEADING_PANEL_HEIGHT, solarClock.getSize().width, solarClock.getSize().height);
      ...
      public void paintComponent(Graphics gr)
      {
        ...
        if (liveWallPaper.booleanValue())// aka Live Wallpaper
        {
          ...
          while (lhaSun < 0) lhaSun += 360; while (lhaSun > 360) lhaSun -= 360;
      ...
      //  System.out.println("Displaying live wall paper.");
       ...
       // Font bgJumboFont = JumboDisplay.tryToLoadFont("TRANGA__.TTF", null);
          ...
          if (false)
          {
            ...
            catch (Exception ex) { System.err.println("Loading font:" + ex.getMessage()); }
          }
          else
          {
            try
...
//                System.err.println("COG in Cache:" + ex.getLocalizedMessage());
  ...
  //                printout("Dec:" + GeomUtil.decToSex(decSun, GeomUtil.SWING, GeomUtil.NS));
                ...
                synchronized (ndc)
                {
                  ...
                  try { tws = ((Speed)ndc.get(NMEADataCache.TWS, true)).getDoubleValue(); } catch (Exception ex) {}
                  try { twd = (int)((Angle360)ndc.get(NMEADataCache.TWD, true)).getDoubleValue(); } catch (Exception ex) {}
...
//          System.out.println("Creating TCP writer on " + rebroadcastPanel.tcpHost() + ":" + TCPPort);
          ...
          else if (!rebroadcastPanel.isTCPSelected() && TCPPort != -1)
          {
            TCPPort = -1;
            try { tcpWriter.close(); }
            catch (Exception ex) { System.err.println("TCPWriter:" + ex.getLocalizedMessage()); }
          }
...
//    try { csp = ((Speed)NMEAContext.getInstance().getCache().get(NMEADataCache.CSP, true)).getDoubleValue(); } catch (Exception ex) {}
      ...
      double sog = -1d;
      int cog = 0;
      try { sog = ((Speed)NMEAContext.getInstance().getCache().get(NMEADataCache.SOG, true)).getDoubleValue(); } catch (Exception ex) {}
      try { cog = (int)((Angle360)NMEAContext.getInstance().getCache().get(NMEADataCache.COG, true)).getDoubleValue(); } catch (Exception ex) {}
