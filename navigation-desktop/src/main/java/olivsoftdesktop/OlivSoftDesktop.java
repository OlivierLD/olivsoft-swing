...
public class OlivSoftDesktop {
    ...
    public OlivSoftDesktop() {
        ...
        try {
            frame.setIconImage(new ImageIcon(this.getClass().getResource("camel.gif")).getImage());
        ...
        if (desktopPos.exists()) {
            ...
            synchronized (parser) {
                try {
                    parser.parse(desktopPos.toURI().toURL());
                } catch (Exception ex) {
...
//            System.out.println(bounds.getX() + "," + bounds.getY() + " " + bounds.getWidth() + " x " + bounds.getHeight());
                    ...
                    if (!foundMatch) {
                        ...
                        } else      // Window is growing up to its final size.
                        {
                            int NB_STEP = 100;
                            for (int i = 0; i <= NB_STEP; i++) {
                                final int _w = (int) (i * (w / (float) NB_STEP));
                                final int _h = (int) (i * (h / (float) NB_STEP));
                                final int _x = (int) (i * (x / (float) NB_STEP));
                                final int _y = (int) (i * (y / (float) NB_STEP));
...
//  frame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE); // To let the smooth close take place
//  frame.setDefaultCloseOperation( JFrame.EXIT_ON_CLOSE );
    ...
    public static void main(String[] args) {
        ...
        if (nbl.exists()) {
            try {
                List<String> noRebroadcastList = new ArrayList<>();
        ...
        } else // Headless Console.
        {
            if (args.length > 0 && args[0].equalsIgnoreCase("-H")) {
                ...
                System.out.println("  In headless mode, Baud rate can be set from the 'baud.rate' system variable,");
                System.out.println("  Example: Use -Dbaud.rate=9600. Default is 4800");
...
//      int resp = JOptionPane.showConfirmDialog(null, guiPanel, "Input-Output", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE);
            ...
            final String logFormat = System.getProperty("log.fmt", "raw");
            final boolean logWithTime = "true".equals(System.getProperty("log.with.time", "false"));

            if (!logFormat.equals("raw") && !logFormat.equals("json")) {
                throw new RuntimeException("Illegal 'log.fmt' value [" + logFormat + "], should be 'raw' or 'json'.");
            }
                    ...
                    NMEAContext.getInstance().addNMEAReaderListener(new NMEAReaderListener("Desktop", "manageNMEAString, headless") {
                        ...
                        public void manageNMEAString(String nmeaString) {
                            ...
                            if (_logFile != null && DesktopContext.getInstance().isFileRebroadcastEnable()) {
                                ...
                                String nmeaData = DesktopUtilities.superTrim(nmeaString);
                                String line = "";
                                if ("raw".equals(logFormat)) {
                                    if (logWithTime)
                                        line = Long.toString(System.currentTimeMillis()) + ":" + nmeaData;
                                    else
                                        line = nmeaData;
                                } else if ("json".equals(logFormat)) {
                                    try {
                                        JSONObject json = new JSONObject();
                                        json.put("nmea", nmeaData);
                                        if (logWithTime)
