...
//            System.out.println("- Cache has " + NMEAContext.getInstance().getCache().size() + " entry(ies).");
  ...
  private void dispatchData(String str)
  {
    if (!StringParsers.validCheckSum(str))
      return;
    if (! str.startsWith("!AIV")) // AIS
    {
      if (! str.startsWith("$") || str.trim().length() < 6 || !str.substring(1, 6).equals(str.substring(1, 6).toUpperCase()))
        return;
    }
//  System.out.println("Substring:[" + payload.substring(1, 6) +"]");
    ...
    try
    {
      ...
      // TODO Manage AIS Strings [!AIVDM]
      if (str.startsWith("!AIV"))
      {
        if ("true".equals(System.getProperty("ais.output", "false")))
          System.out.println("AIS:" + AISParser.parseAIS(str)); // Temporary...
      }
    }
    catch (Exception ex)
    {
