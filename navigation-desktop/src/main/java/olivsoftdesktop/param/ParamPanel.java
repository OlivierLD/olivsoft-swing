...
import olivsoftdesktop.param.widget.*;
    ...
    private static Object oneFactorySetting(int settingID) {
        ...
        switch (settingID) {
            ...
            case ParamData.AIRMAIL_ID:
                it = "- No ID -";
                break;
            case ParamData.SHOW_WEB_W_TEMP:
            case ParamData.SHOW_WEB_A_TEMP:
                it = Boolean.valueOf(true);
                break;
            case ParamData.SHOW_WEB_GDT:
            case ParamData.SHOW_WEB_PRMSL:
            case ParamData.SHOW_WEB_HUM:
            case ParamData.SHOW_WEB_VOLT:
            case ParamData.NMEA_AT_STARTUP:
            case ParamData.LWP_AT_STARTUP:
            case ParamData.LWP_GLOSSY_DISPLAY:
                it = Boolean.valueOf(false);
                break;
            case ParamData.DEFAULT_LAT:
                it = new Double(37.74888723023488);  // SF, Ocean beach
                break;
            case ParamData.DEFAULT_LON:
                it = new Double(-122.50712603330612); // SF, Ocean beach
                break;
            default:
                break;
        }
                    ...
                    try {
                        ...
                        if (i == ParamData.NMEA_CHANNEL)
                            ...
                            data[i][PRM_VALUE] = new DataDirectory("DB Files Location", s);
                                ...
                                i == ParamData.DEFAULT_LAT ||
                                i == ParamData.DEFAULT_LON)
                            ...
                            data[i][PRM_VALUE] = new Float(s);
                                ...
                                i == ParamData.SHOW_WEB_W_TEMP ||
                                i == ParamData.SHOW_WEB_A_TEMP ||
                                i == ParamData.SHOW_WEB_GDT ||
                                i == ParamData.SHOW_WEB_PRMSL ||
                                i == ParamData.SHOW_WEB_HUM ||
                                i == ParamData.SHOW_WEB_VOLT ||
                                i == ParamData.NMEA_AT_STARTUP ||
                                i == ParamData.LWP_AT_STARTUP ||
                                i == ParamData.LWP_GLOSSY_DISPLAY) {
                            Boolean b = new Boolean(s);
                            data[i][PRM_VALUE] = b;
                            if (i == ParamData.SHOW_WEB_W_TEMP ||
                                    i == ParamData.SHOW_WEB_A_TEMP ||
                                    i == ParamData.SHOW_WEB_GDT ||
                                    i == ParamData.SHOW_WEB_PRMSL ||
                                    i == ParamData.SHOW_WEB_HUM ||
                                    i == ParamData.SHOW_WEB_VOLT) // Those live in the cache
                            {
                                NMEADataCache cache = NMEAContext.getInstance().getCache();
                                synchronized (cache) {
                                    switch (i) {
                                        case ParamData.SHOW_WEB_W_TEMP:
                                            cache.put(NMEADataCache.DISPLAY_WEB_WT, b);
                                            break;
                                        case ParamData.SHOW_WEB_A_TEMP:
                                            cache.put(NMEADataCache.DISPLAY_WEB_AT, b);
                                            break;
                                        case ParamData.SHOW_WEB_GDT:
                                            cache.put(NMEADataCache.DISPLAY_WEB_GDT, b);
                                            break;
                                        case ParamData.SHOW_WEB_PRMSL:
                                            cache.put(NMEADataCache.DISPLAY_WEB_PRMSL, b);
                                            break;
                                        case ParamData.SHOW_WEB_HUM:
                                            cache.put(NMEADataCache.DISPLAY_WEB_HUM, b);
                                            break;
                                        case ParamData.SHOW_WEB_VOLT:
                                            cache.put(NMEADataCache.DISPLAY_WEB_VOLT, b);
                                            break;
                                        default:
                                            break;
                                    }
...
//        System.out.println("** setUserValue: i=" + Integer.toString(i) + " : [" + data[i][PRM_VALUE].toString() + "], a [" + data[i][PRM_VALUE].getClass().getName() + "] for [" + ParamData.labels[i] + "].");
                    ...
                    new int[] // NMEA
                            {
                                    ...
                                    ParamData.NMEA_AT_STARTUP,
                                    ParamData.MAX_ANALOG_BSP,
                                    ParamData.MAX_ANALOG_TWS,
                                    ParamData.SHOW_WEB_W_TEMP,
                                    ParamData.SHOW_WEB_A_TEMP,
                                    ParamData.SHOW_WEB_GDT,
                                    ParamData.SHOW_WEB_PRMSL,
                                    ParamData.SHOW_WEB_HUM,
                                    ParamData.SHOW_WEB_VOLT    /*,
    ...
    public static Object[] getPrefsByCategory() {
...
//  List<String> reloadDataRequired = new ArrayList<String>();
        ...
        if (currentCategoryIndex > -1) {
            for (int i = 0; i < localData.length; i++) {
                String before = "";
                try {
                    before = (data[((int[]) categoryIndexes[currentCategoryIndex])[i]][PRM_VALUE]).toString();
                } catch (Exception ex) {
...
//        g.drawString((String)curValue, 1, getHeight() - 1);
    ...
    public static class MarqueeDataList {
        ...
        public MarqueeDataList(String s) {
            this.value = s;
        }
