  ...
  implements UserExitInterface
{
  ...
  private int motes = 50000; 
  private int wsFactor = 5; // Wind speed factor
  private long betweenLoops = 2000L;
...
//    available = false;
    if (ctx.getCurrentGribFileName() == null || ctx.getCurrentGribFileName().trim().length() == 0 || !commandPanel.isThereWind())
      available = false;
    return available;
  }

  private final void showDustlet(final Thread parent, String gribFile)
  {
    if (gribFile != null)
    {
      try
...
//    System.out.println("...Done");
      JFrame frame = new JFrame("GRIB Dustlet");
...
//          }
  ...
  private final int[] generateDustletFile(GribHelper.GribConditionData gribData,
                                          String fileName,
                                          int value) throws Exception
  {
    ...
    try { dustletPanel.setDustletDate(gribData.getDate()); } catch (Exception ignore) { System.out.println(ignore.toString()); }
    ...
    for (int h=gribData.getGribPointData().length - 1; h>=0; h--)
    {
      ...
      for (int w=0; w<gribData.getGribPointData()[h].length; w++)
      {
             ...
             GRIBDataUtil.isBetween(topLeft.getG(), bottomRight.getG(), gribData.getGribPointData()[h][w].getLng()))
        {
          ...
          line += (Integer.toString(value * Math.round(x)) + " ");
        }
      }
    ...
    for (int h=gribData.getGribPointData().length - 1; h>=0; h--)
    {
      ...
      for (int w=0; w<gribData.getGribPointData()[h].length; w++)
      {
             ...
             GRIBDataUtil.isBetween(topLeft.getG(), bottomRight.getG(), gribData.getGribPointData()[h][w].getLng()))
        {
          ...
          line += (Integer.toString(value * Math.round(y)) + " ");
        }        
      }
      if (line.trim().length() > 0)
      {
