...
//      System.out.println("For " + d.toString() + " and type " + t + " (" + t.getDesc() + ", " + t.getUnit() + ")");          
      ...
      {
        if (verbose) System.out.println("PreviousData2Display set to " + (data2display==null?"null":"not null"));
          ...
          public void run()
          {
            ...
            while (slug)
            {
              if (smoothingStep == -1 || smoothingStep >= smoothingSteps)
              {
                ...
                if (verbose) System.out.println("*** Preparing Data (" + (prevDate != null?prevDate.toString():"-null-") + ") ***");
